...
WORKERS = 10  # Максимальное количество одновременно выполняемых задач
...
async def process_tags(session: ClientSession, post: Dict[str, str]) -> Dict[str, int]:
    ...
    tags_dict = {}
    # print('Работаем с новыми тегами')
        ...
        while count < 5:
            ...
            async with session.post(f"{URL}/posts", auth=BasicAuth(user, password), json=data) as resp:
                try:
                    ...
                    logger.error(f"Ошибка создания поста '{post['name']}': \n{ex}")
                    count += 1
                    await asyncio.sleep(5)
        return None
...
async def process_single_post(session: ClientSession, post: Dict[str, str]) -> List[str]:
    ...
    while count < 5:
        try:
            ...
            logger.error(f"Общая ошибка создания поста '{post['name']}': \n{ex}")
            count += 1
            await asyncio.sleep(5)
...
async def process_posts():
    ...
    csv_file = 'layer_styles'
    post_data = get_data(csv_file)
    result_data = [['category', 'name', 'description', 'tags', 'img_path', 'post_link', 'product_link']]
    # result_data = [['category', 'name', 'description', 'img_path', 'post_link', 'product_link']]
